...
public class AuthService {
    ...
    public AuthResponse login(LoginRequest request, ClientInfo clientInfo) {
        validateIdentifier(request.identifierType(), request.identifier());
        String identifier = normalizeIdentifier(request.identifierType(), request.identifier());
        String ip = clientInfo != null ? clientInfo.ip() : null;

        // 先过风控
        loginRiskControlService.assertLoginAllowed(identifier, ip);

        Optional<User> userOptional = findUserByIdentifier(request.identifierType(), identifier);

        // 账号不存在也记风控失败，避免被枚举绕过
        if (userOptional.isEmpty()) {
            loginRiskControlService.recordFailure(identifier, ip);
            loginLogService.record(null, identifier, "UNKNOWN", ip, clientInfo != null ? clientInfo.userAgent() : null, "FAILED");
        ...
        try {
            ...
            // 登录成功，清理账号维度和账号+IP 维度的失败状态
            loginRiskControlService.clearOnSuccess(identifier, ip);

            TokenPair tokenPair = jwtService.issueTokenPair(user);
            storeRefreshToken(user.getId(), tokenPair);
            loginLogService.record(user.getId(), identifier, StringUtils.hasText(request.password()) ? "PASSWORD" : "CODE", ip,
                    clientInfo != null ? clientInfo.userAgent() : null, "SUCCESS");
            return new AuthResponse(mapUser(user), mapToken(tokenPair));
        } catch (BusinessException ex) {
            if (ex.getErrorCode() == ErrorCode.INVALID_CREDENTIALS
                    || ex.getErrorCode() == ErrorCode.VERIFICATION_NOT_FOUND
                    || ex.getErrorCode() == ErrorCode.VERIFICATION_MISMATCH
                    || ex.getErrorCode() == ErrorCode.VERIFICATION_TOO_MANY_ATTEMPTS) {
                loginRiskControlService.recordFailure(identifier, ip);
                loginLogService.record(user.getId(), identifier, StringUtils.hasText(request.password()) ? "PASSWORD" : "CODE",
                        ip, clientInfo != null ? clientInfo.userAgent() : null, "FAILED");
            }
            throw ex;
        }
    }
    ...
    public TokenResponse refresh(TokenRefreshRequest request) {
        ...
        if (!refreshTokenStore.isTokenValid(userId, oldTokenId)) {
            throw new BusinessException(ErrorCode.REFRESH_TOKEN_INVALID);
        }
        ...
        Duration newTtl = Duration.between(Instant.now(), tokenPair.refreshTokenExpiresAt());
    ...
    public void logout(String refreshToken,Jwt accessJwt) {
        ...
        if (accessJwt != null && jwtService.isAccessToken(accessJwt)) {
            String jti = jwtService.extractTokenId(accessJwt);
            Instant expiresAt = accessJwt.getExpiresAt();
            if (expiresAt != null) {
                Duration ttl = Duration.between(Instant.now(), expiresAt);
                accessTokenBlacklistStore.blacklist(jti, ttl);
            }
        }
    }
    ...
    public void resetPassword(PasswordResetRequest request) {
        ...
        Duration validAfterTtl = authProperties.getJwt().getAccessTokenTtl().plus(Duration.ofMinutes(1));
        userAccessTokenValidityStore.setValidAfter(user.getId(), now, validAfterTtl);
        refreshTokenStore.revokeAllByScan(user.getId());
    }
