...
public interface RefreshTokenStore {
    ...
    /**
     * 原子轮换 refresh token：
     * 只有旧 token 仍有效时，才会原子地：
     * 1. 删除旧 token
     * 2. 写入新 token
     *
     * @param userId     用户 ID
     * @param oldTokenId 旧 refresh token 的 jti
     * @param newTokenId 新 refresh token 的 jti
     * @param newTtl     新 refresh token 的 TTL
     * @return 成功返回 true；旧 token 已失效/已被消费则返回 false
     */
    boolean rotateToken(long userId, String oldTokenId, String newTokenId, Duration newTtl);

    void revokeAllByScan(long userId);
}
