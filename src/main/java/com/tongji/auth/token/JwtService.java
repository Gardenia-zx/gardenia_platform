...
public class JwtService {
    ...
    private static final String CLAIM_EXPIRES_AT = "expires_at";
    ...
    public boolean isAccessToken(Jwt jwt){
        return StringUtil.equals("access",extractTokenType(jwt));
    }

    public boolean isRefreshToken(Jwt jwt){
        return StringUtil.equals("refresh",extractTokenType(jwt));
    }

    public Instant extractIssuedAt(Jwt jwt) {
        Instant issuedAt = jwt.getIssuedAt();
        if (issuedAt == null) {
            throw new IllegalArgumentException("Missing iat in token");
        }
        return issuedAt;
    }
}
