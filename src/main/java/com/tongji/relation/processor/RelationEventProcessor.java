...
public class RelationEventProcessor {
    ...
    public void process(RelationEvent evt) {
        String doneKey = "done:rel:" + evt.type() + ":" + evt.fromUserId() + ":" + evt.toUserId() + ":" + (evt.id() == null ? "0" : String.valueOf(evt.id()));
        String processKey = "process:rel:" + evt.type() + ":" + evt.fromUserId() + ":" + evt.toUserId() + ":" + (evt.id() == null ? "0" : String.valueOf(evt.id()));
        boolean done = redis.hasKey(doneKey);
        if(done){
            log.info("Relation event already processed: {}", evt);
            return;
        }
//        获取锁
        Boolean locked = redis.opsForValue().setIfAbsent(processKey,"1", Duration.ofMinutes(1));
        if(locked==null||!locked){
            throw new BusinessException(ErrorCode.RELATION_EVENT_PROCESSING_LOCKED);
        }
        try {
...
//            加入到done锁中，标记为成功处理
            redis.opsForValue().set(doneKey, "1",Duration.ofDays(1));
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
//            释放process锁
            redis.delete(processKey);
        }
    }
}
