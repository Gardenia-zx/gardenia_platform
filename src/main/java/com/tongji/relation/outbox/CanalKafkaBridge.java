...
public class CanalKafkaBridge implements SmartLifecycle {
     ...
     */
//    start()spring的一个钩子函数，当spring项目启动时，扫描到这个类会调用这个start()方法
    @Override
    public void start() {
//        判断是否在运行状态，防止多次重复建立canal连接
        if (running) {
        ...
        // 标记运行并使用全局线程池异步执行主循环
        running = true;
//       在java线程池里面执行这个任务，不会阻塞spring主线程启动
        taskExecutor.execute(() -> {
            try {
                ...
                while (running) {
                    ...
                    // 空批次或心跳时，按间隔休眠并继续轮询，防止重复轮询占用cpu
                    ...
                    for (CanalEntry.Entry entry : message.getEntries()) {
                        ...
                        try {
                            ...
                            batchSuccess = false;
                            log.error("Canal bridge error: dest={} err={}", OutboxTopics.CANAL_OUTBOX, ignored.getMessage());
                        }
                    }
