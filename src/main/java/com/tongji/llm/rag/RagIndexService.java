...
public class RagIndexService {
    ...
    public int reindexSinglePost(long postId) {
        ...
        // 指纹检测：如未变化则跳过重建
//        contentSha256：正文内容的哈希摘要，更可靠
//        contentEtag：HTTP 资源版本标识，也可以用来判断文件是否变化
        ...
        for (int i = 0; i < chunks.size(); i++) {
            ...
            meta.put("postId", String.valueOf(postId));//检索文章过滤
            meta.put("chunkId", cid);//分片id，更新切片
            meta.put("position", i);//顺序
            meta.put("contentEtag", currentEtag);//
            meta.put("contentSha256", currentSha);//
            meta.put("contentUrl", row.getContentUrl());//正文存储地址
            meta.put("title", row.getTitle());//标题
